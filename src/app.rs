                ...
                match self.section {
                    ...
                    Section::Kube => {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            vscode_combo_id("kube_ns_global")
                            ...
                            vscode_combo_id("kube_cluster_global")
...
impl CsvTab {
    ...
    fn show(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            vscode_combo_id(("csv_delim", &salt))
        ...
        if self.ready() {
            ...
            ui.horizontal(|ui| {
                ...
                vscode_combo_id(("csv_filter_col", &salt))
    ...
    fn import_view(&mut self, ui: &mut egui::Ui) {
        ...
        egui::CentralPanel::default_margins().show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                vscode_combo_id("import_table_pick")
        ...
        if let Some(d) = &mut self.import_dialog {
            ...
            ui.separator();
            {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        vscode_combo_id("import_encoding")
                        ...
                        vscode_combo_id("import_delimiter")
        ...
        egui::CentralPanel::default_margins().show(ui, |ui| {
            ...
            if self.verify.is_none() {
                ...
                ui.horizontal(|ui| {
                    ...
                    vscode_combo_id("verify_table_pick")
                        .selected_text(sel_text)
                        .width(280.0)
                        .show_ui(ui, |ui| {
                            if tables.is_empty() {
                        ...
                        egui::Grid::new("verify_map")
                            ...
                            .show(ui, |ui| {
                                ...
                                for (i, (name, ty, pk)) in cols.iter().enumerate() {
                                    ...
                                    vscode_combo_id(("verify_map_combo", i))
                                        .selected_text(sel_text)
                                        .width(240.0)
                                        .show_ui(ui, |ui| {
...
fn combo_chevron(ui: &egui::Ui, rect: egui::Rect, visuals: &egui::style::WidgetVisuals, _open: bool) {
    let painter = ui.painter();
    let c = rect.center();
    let w = 3.3_f32; // 横の半幅
    let h = 2.0_f32; // V の高さ（半分）
    let stroke = egui::Stroke::new(1.3, visuals.fg_stroke.color);
    painter.line_segment([egui::pos2(c.x - w, c.y - h), egui::pos2(c.x, c.y + h)], stroke);
    painter.line_segment([egui::pos2(c.x + w, c.y - h), egui::pos2(c.x, c.y + h)], stroke);
}

/// プルダウンを VS Code 風（細いシェブロン・フラット）で作るヘルパ。
/// 既存の `ComboBox::from_id_salt(..)` を置き換えて全箇所で統一する。
fn vscode_combo_id(id_salt: impl egui::AsIdSalt) -> egui::ComboBox {
...
fn setup_style(ctx: &egui::Context) {
    ...
    ctx.all_styles_mut(|s| {
        // VS Code に合わせる: UI は 13px。ウィジェットは縦に十分な高さを取り、
        // テキストを上下中央に置く（上寄りに見えないように padding を対称化）。
    ...
    fn render_font_alignment_check() {
        ...
        impl eframe::App for FontApp {
            fn ui(&mut self, ui: &mut egui::Ui, _: &mut eframe::Frame) {
                ...
                // プルダウン + ボタン + 入力欄の縦中央寄せ確認。
                ui.horizontal(|ui| {
                    let _ = ui.button("照合を実行 Run 99 件");
                    vscode_combo_id("demo_combo")
                        .selected_text(["UTF-8 漢字", "Shift-JIS", "カンマ ,"][self.sel])
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut self.sel, 0, "UTF-8 漢字");
                            ui.selectable_value(&mut self.sel, 1, "Shift-JIS");
                            ui.selectable_value(&mut self.sel, 2, "カンマ ,");
                        });
                    ui.add(egui::TextEdit::singleline(&mut self.text).hint_text("入力 abc 漢字"));
                });
